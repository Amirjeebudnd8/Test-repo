        ...
        try {
            ...
            for (let i = 20; i < marketData.length; i++) {
                ...
                try {
                    ...
                    // ========== اصلاحات: تعریف sharpTrendsParam و اضافه کردن پارامتر پنجم ==========
...
function getSharpTrendsAtCandle(candleIndex) {
    ...
    return sharpTrends.filter(trend => 
        ...
        `);
                    ...
                    // ========== اصلاح: ارسال sharpTrendsParam به عنوان پارامتر پنجم ==========
                    const signal = strategyFn(marketData, i, combinedBreakParams, ichimoku, sharpTrendsParam);
        ...
        try {
            ...
            for (let i = 20; i < sampleSize; i++) {
                try {
                    ...
                    const strategyFn = new Function('data', 'index', 'ichimokuParam', 'sharpTrendsParam', `
                        function calculateSMA(data, endIndex, period) {
                            let sum = 0;
                            for (let i = 0; i < period; i++) {
                                if (endIndex - i < 0) return null;
                    ...
                    const signal = strategyFn(marketData, i, ichimoku, options.sharpTrends || []);
