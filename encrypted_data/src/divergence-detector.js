...
// ======================== بخش ۸: نسخه Node.js مستقیم برای استفاده در run-backtest.js ========================
function detectDivergenceForBacktest(marketData, indicatorType = 'RSI') {
    if (!marketData || !Array.isArray(marketData) || marketData.length < 30) {
        console.error('[DIVERGENCE] ❌ marketData نامعتبر است.');
        return [];
    }
    if (!['RSI', 'MACD'].includes(indicatorType)) {
        console.error('[DIVERGENCE] ❌ indicatorType باید "RSI" یا "MACD" باشد.');
        return [];
    }

    const fakeSendMessage = (msg) => {
        if (msg.type === 'DIVERGENCE_RESULT') {
            console.log(`[DIVERGENCE] 📬 ${msg.payload.signals.length} سیگنال آماده شد.`);
        }
    };

    return runDivergenceDetection({
        marketData,
