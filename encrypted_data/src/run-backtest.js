...
const MAX_DURATION_MS = 20 * 1000; // ۲۰ ثانیه برای تست - بعداً به 330 * 60 * 1000 تغییر دهید
let timerExpired = false;

setTimeout(() => {
  timerExpired = true;
  console.log(`⏰ زمان (${MAX_DURATION_MS / 1000} ثانیه) به پایان رسید. در حال خاموش کردن و ذخیره وضعیت...`);
}, MAX_DURATION_MS);
...
function updateCompletedChunk(strategyName, chunkIndex, progress) {
  ...
  // تلاش برای commit و push فایل وضعیت به مخزن (برای هماهنگی بین ردفلوها)
...
(async () => {
  ...
  // قبل از شروع، بررسی کنیم که آیا این چانک قبلاً کامل شده است؟
  ...
  // آدرس فایل وضعیت و اطلاعات زمان در options برای دسترسی داخل backtest-core (اگر نیاز شود)
  backtestOptions.startTime = START_TIME;
  backtestOptions.maxDurationMs = MAX_DURATION_MS;
  ...
  // قبل از ذخیره نتایج، بررسی کنیم که آیا زمان منقضی شده
  if (timerExpired) {
    console.log(`⚠️ زمان به پایان رسیده و چانک ${currentChunk} کامل نشده است. لغو می‌شود.`);
    // بدون ذخیره نتایج و بدون به‌روزرسانی فایل وضعیت خارج می‌شویم
    process.exit(0);
  }
  ...
  // اگر اینجا رسیدیم یعنی چانک با موفقیت کامل شده است. پس آن را در فایل وضعیت علامت بزن.
