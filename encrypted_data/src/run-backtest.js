...
// ==================== تابع رمزنگاری نتایج (یکپارچه - فقط برای backward compatibility) ====================
...
(async () => {
  ...
  // 5. اجرای بکتست با پارامترهای اجباری از استراتژی
  let backtestOptions = {
    code: strategyCode,
    ...
    // تنظیمات پیش‌فرض (در صورت عدم ارائه توسط استراتژی، بعداً بررسی و خطا می‌دهیم)
    ichimoku: null,
    trendlineSettings: null,
    divergenceSettings: null,
    sharpTrendsSettings: null
  };

  // تابع برای استخراج تنظیمات از استراتژی
  function extractStrategySettings(strategyCode, marketData, sampleIndex) {
    // تعریف توابع کمکی داخل eval
    const helper = `
      function calculateSMA(data, endIndex, period) { /* ... */ }
      function getTrendLines() { return ${JSON.stringify(trendLines)}; }
      function getSharpTrends() { return ${JSON.stringify(sharpTrends)}; }
      ${strategyCode}
      return customStrategy(marketData, sampleIndex);
    `;
    try {
      const func = new Function('marketData', 'sampleIndex', helper);
      const strategyOutput = func(marketData, sampleIndex);
      return strategyOutput || {};
    } catch (err) {
      console.error('Error extracting strategy settings:', err);
      return {};
    }
  }
  ...
  const requiredParams = [
    ...
    { path: 'ichimoku.senkouBPeriod', type: 'number' },
    { path: 'ichimoku.useCloudFilter', type: 'boolean' },
    { path: 'ichimoku.useTKCross', type: 'boolean' },
    { path: 'ichimoku.useChikou', type: 'boolean' },
    { path: 'trendlines.pivotPeriod', type: 'number' },
    { path: 'trendlines.minTouchPoints', type: 'number' },
    { path: 'trendlines.minCandleDistance', type: 'number' },
    { path: 'trendlines.maxDeviationPercent', type: 'number' },
    { path: 'divergence.rsiPeriod', type: 'number' },
    { path: 'divergence.macdFastPeriod', type: 'number' },
    { path: 'divergence.macdSlowPeriod', type: 'number' },
    { path: 'divergence.macdSignalPeriod', type: 'number' },
    { path: 'divergence.macdUseHistogram', type: 'boolean' },
    { path: 'sharpTrends.consecutiveCandles', type: 'number' },
    { path: 'sharpTrends.minPercentChange', type: 'number' },
    { path: 'sharpTrends.boxValidityHours', type: 'number' }
  ];

  let missingParams = [];
  for (const param of requiredParams) {
    const parts = param.path.split('.');
    let value = strategySettings;
    for (const part of parts) {
      if (value === undefined || value === null) break;
      value = value[part];
    }
    if (value === undefined || value === null) {
      missingParams.push(param.path);
    } else if (param.type === 'number' && typeof value !== 'number') {
      missingParams.push(`${param.path} (expected number, got ${typeof value})`);
    } else if (param.type === 'boolean' && typeof value !== 'boolean') {
      missingParams.push(`${param.path} (expected boolean, got ${typeof value})`);
    } else {
      // مقداردهی به backtestOptions
      if (param.path.startsWith('ichimoku.')) {
        if (!backtestOptions.ichimoku) backtestOptions.ichimoku = {};
        const key = param.path.split('.')[1];
        backtestOptions.ichimoku[key] = value;
      } else if (param.path.startsWith('trendlines.')) {
        if (!backtestOptions.trendlineSettings) backtestOptions.trendlineSettings = {};
        const key = param.path.split('.')[1];
        backtestOptions.trendlineSettings[key] = value;
      } else if (param.path.startsWith('divergence.')) {
        if (!backtestOptions.divergenceSettings) backtestOptions.divergenceSettings = {};
        const key = param.path.split('.')[1];
        backtestOptions.divergenceSettings[key] = value;
      } else if (param.path.startsWith('sharpTrends.')) {
        if (!backtestOptions.sharpTrendsSettings) backtestOptions.sharpTrendsSettings = {};
        const key = param.path.split('.')[1];
        backtestOptions.sharpTrendsSettings[key] = value;
      }
    }
  }

  if (missingParams.length > 0) {
    console.error('❌ خطا: پارامترهای اجباری زیر در خروجی استراتژی ارائه نشده‌اند:');
    missingParams.forEach(p => console.error(`   - ${p}`));
    console.error('لطفاً استراتژی خود را اصلاح کنید تا این پارامترها را برگرداند.');
    process.exit(1);
  }

  // اضافه کردن تنظیمات به backtestOptions
  backtestOptions.ichimoku = {
    enabled: true,
    useCloudFilter: backtestOptions.ichimoku.useCloudFilter,
    useTKCross: backtestOptions.ichimoku.useTKCross,
    useChikou: backtestOptions.ichimoku.useChikou,
    tenkanPeriod: backtestOptions.ichimoku.tenkanPeriod,
    kijunPeriod: backtestOptions.ichimoku.kijunPeriod,
    senkouBPeriod: backtestOptions.ichimoku.senkouBPeriod
  };

  // تنظیمات خطوط روند (برای backtest-core که از آنها استفاده می‌کند)
  if (backtestOptions.trendlineSettings) {
    // مقادیر را در options قرار می‌دهیم تا backtest-core از آنها استفاده کند
    backtestOptions.pivotPeriod = backtestOptions.trendlineSettings.pivotPeriod;
    backtestOptions.minTouchPoints = backtestOptions.trendlineSettings.minTouchPoints;
    backtestOptions.minCandleDistance = backtestOptions.trendlineSettings.minCandleDistance;
    backtestOptions.maxDeviationPercent = backtestOptions.trendlineSettings.maxDeviationPercent;
  }

  // تنظیمات واگرایی (برای divergence-detector)
  if (backtestOptions.divergenceSettings && divergenceDetector) {
    // می‌توان مستقیما به شیء سراسری اختصاص داد، اما برای سادگی فعلاً ignore می‌کنیم
    // divergence-detector از const DIVERGENCE_CONFIG استفاده می‌کند که در ماژول تعریف شده.
    // برای تغییر آن نیاز به تغییر در ماژول داریم. در اینجا فقط اطلاع می‌دهیم.
    console.log('ℹ️ تنظیمات واگرایی از استراتژی دریافت شد، اما ماژول divergence-detector نیاز به تغییر دارد تا از آنها استفاده کند.');
  }

  // تنظیمات روند شارپ (برای تابع detectSharpTrends که قبلاً از SHARP سراسری استفاده می‌کند)
  if (backtestOptions.sharpTrendsSettings) {
    SHARP.consecutiveCandles = backtestOptions.sharpTrendsSettings.consecutiveCandles;
    SHARP.minPercentChange = backtestOptions.sharpTrendsSettings.minPercentChange;
    SHARP.boxValidityHours = backtestOptions.sharpTrendsSettings.boxValidityHours;
    // دوباره روندهای شارپ را با تنظیمات جدید محاسبه می‌کنیم
    const updatedSharpTrends = detectSharpTrends(marketData);
