...
// ==================== تنظیمات ====================
const CHUNK_SIZE = 10;                 // تعداد فایل در هر چانک
...
const { detectTrendLinesAdvanced } = require('./trendline-detector.js');
const { calculateIchimoku } = require('./ichimoku-calculator.js');
const { generateBreakPoints } = require('./breakpoint-generator.js');
const { detectDivergenceForBacktest } = require('./divergence-detector.js');
...
// ==================== استخراج ANALYSIS_CONFIG از استراتژی ====================
function extractAnalysisConfig(code) {
    try {
        const fn = new Function(`
            ${code}
            return typeof ANALYSIS_CONFIG !== 'undefined' ? ANALYSIS_CONFIG : undefined;
        `);
        const config = fn();
        if (!config || typeof config !== 'object') {
            throw new Error('ANALYSIS_CONFIG is not defined or invalid in strategy.');
        }
        return config;
    } catch (err) {
        console.error('❌ Failed to extract ANALYSIS_CONFIG:', err.message);
        process.exit(1);
    }
}
...
// ==================== تابع تجمیع معاملات یک فایل ====================
async function processSingleFile(fileName, strategyCode, backtestCore, resultsPassword, nextFileName = null) {
  ...
  if (nextFileName) {
      nextFileData = loadSingleFile(dataDir, nextFileName);
      if (nextFileData.length === 0) nextFileData = null;
  }
  ...
  // اجرای تحلیل‌ها بر اساس ANALYSIS_CONFIG
  let trendLines = null, ichimokuData = null, breakPointsMap = null, divergenceSignals = null;

  if (analysisConfig.trendLines) {
      const res = detectTrendLinesAdvanced(marketData, analysisConfig.trendLines);
      if (res.error) throw new Error(`TrendLines: ${res.error}`);
      trendLines = res.trendLines;
  }
  if (analysisConfig.ichimoku) {
      const res = calculateIchimoku(marketData, analysisConfig.ichimoku);
      if (res.error) throw new Error(`Ichimoku: ${res.error}`);
      ichimokuData = res.result;
  }
  if (analysisConfig.breakPoints) {
      if (!trendLines && analysisConfig.trendLines) {
          const res = detectTrendLinesAdvanced(marketData, analysisConfig.trendLines);
          if (res.error) throw new Error(`TrendLines (for BP): ${res.error}`);
          trendLines = res.trendLines;
      }
      if (!trendLines) throw new Error('Trend lines required for break points');
      const res = generateBreakPoints(marketData, trendLines, analysisConfig.breakPoints);
      if (res.error) throw new Error(`BreakPoints: ${res.error}`);
      breakPointsMap = res.breakPoints;
  }
  if (analysisConfig.divergence) {
      divergenceSignals = detectDivergenceForBacktest(marketData, analysisConfig.divergence.indicatorType || 'RSI');
  }
...
(async () => {
  ...
  for (let idx = 0; idx < filesInChunk.length; idx++) {
    ...
    const nextFile = (idx + 1 < filesInChunk.length) ? filesInChunk[idx + 1] : null;
    const res = await processSingleFile(fileName, strategyCode, backtestCore, resultsPassword, nextFile);
    if (res.expired) {
      anyExpired = true;
      break;
    }
    ...
    combinedFinalCapital = res.finalCapital;
  }
