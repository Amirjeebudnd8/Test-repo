console.log('✅ نسخه نهایی – بدون مقادیر پیش‌فرض');
const fs = require('fs');
...
log(`شروع run-backtest.js با آرگومان‌ها: strategyFile=${strategyFile}, startIndex=${startIndex}, dataDir=${dataDir}`);
...
function extractAnalysisConfig(code) {
    try {
        // روش ایمن: اجرا در یک تابع و برگرداندن ANALYSIS_CONFIG
        ...
        throw err; // دوباره پرتاب می‌کنیم تا در بلوک اصلی catch شود
    }
}
...
function getFileNameByIndex(dataDir, index) {
    ...
    if (index >= dated.length) {
        log(`ایندکس ${index} خارج از محدوده است. تعداد فایل‌ها: ${dated.length}`, 'ERROR');
        process.exit(1);
    }
...
log(`فایل انتخاب‌شده: ${fileName}`);

const outputDir = path.join(process.cwd(), 'results', strategyName, fileName);
if (fs.existsSync(path.join(outputDir, 'results.enc'))) {
    log(`⏩ فایل ${fileName} قبلاً انجام شده است.`);
    process.exit(0);
}
...
// ====================== بخش روند شارپ ======================
const SHARP = {
    minCandlesRequired: 5, consecutiveCandles: 5, minPercentChange: 0.5,
    maxLookback: 2500, enableOppositeCandleRule: true,
    boxValidityHours: 72, useFixedDuration: true, breakThresholdPercent: 10.0
};
...
function detectSequenceFromPosition(startIdx, data, config) {
    ...
    const s = data[startIdx]; const trend = candleColor(s);
    ...
    for (let i = startIdx + 1; i <= max; i++) {
        ...
        if (candleColor(c) === trend) { endIdx = i; lastSame = i; }
            ...
            if (config.enableOppositeCandleRule) {
                ...
                if (trend === 'bullish') { if (c.low >= prev.low) { oppCnt++; continue; } }
                else { if (c.high <= prev.high) { oppCnt++; continue; } }
            }
            break;
        }
    }
    ...
    for (let i = startIdx; i <= endIdx; i++) {
        if (candleColor(data[i]) === trend ||
            ...
            hh = Math.max(hh, data[i].high); ll = Math.min(ll, data[i].low);
        }
    }
...
(async () => {
    ...
    if (needsTrendLines) {
        log('🟡 در حال تشخیص خطوط روند...');
        try {
            const trendRes = await backtestCore.detectTrendLinesAdvanced(marketData, analysisConfig.trendLines);
            if (trendRes.error) {
                log('❌ خطا در تشخیص خطوط روند: ' + trendRes.error, 'ERROR');
                process.exit(1);
            }
            trendLines = trendRes.trendLines;
            log(`✅ ${trendRes.statistics.totalLines} خط روند یافت شد.`);
        } catch (err) {
            log('❌ استثناء در تشخیص خطوط روند: ' + err.message, 'ERROR');
            process.exit(1);
        }
    } else {
    ...
    log(`بک‌تست پایان یافت. تعداد معاملات: ${result.trades ? result.trades.length : 0}`);
    ...
    log('گزارش detailed_results.json ذخیره شد.');
    ...
    log('گزارش trades_summary.json ذخیره شد.');

    const stratDir = path.join(process.cwd(), 'results', strategyName);
